...
//                throw invalid_iterator();
	...
	iterator insert(iterator pos, const T &value) {
        ...
        if (pos.theblock->thissize == M){
            ...
            iterator add;
            add.thedeque = pos.thedeque;
            if (pos.theblock->next == pos.thedeque->tail || pos.theblock->next->thissize == M){
                add.theblock = new block;
                add.theblock->thissize++;
                add.theblock->pre = pos.theblock;
                add.theblock->next = pos.theblock->next;
                pos.theblock->next->pre = add.theblock;
                pos.theblock->next = add.theblock;
                node* newone = new node;
                newone->data = new T(*(tem->data));
                newone->pre = add.theblock->chain;
                add.theblock->chain->next = newone;
                tem->pre->next = NULL;
                delete tem;
                pos.theblock->thissize--;
            }
            else{
                add.theblock = pos.theblock->next;
                add.thenode = pos.theblock->next->chain->next;
                insert(add, *(tem->data));
                delete tem;
                pos.theblock->thissize--;
                pos.theblock->next->thissize++;
            }
        }
        totalsize++;
        pos.theblock->thissize++;
