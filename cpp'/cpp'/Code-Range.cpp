...
int binary_search_for_m(vector<long>& sum, long sum_i, int LOWER, int low, int high) {
    ...
    if (sum[mid] - sum_i >= LOWER)
        ans = binary_search_for_m(sum, sum_i, LOWER, low, mid);
    else
        ans = binary_search_for_m(sum, sum_i, LOWER, mid, high);
    if (sum[ans] - sum_i >= LOWER)
        return ans;
    else
        return high;
	/*
...
int binary_search_for_n(vector<long>& sum, long sum_i, int UPPER, int low, int high) {
    ...
    if (sum[mid] - sum_i > UPPER)
        ans = binary_search_for_n(sum, sum_i, UPPER, low, mid);
    else
        ans = binary_search_for_n(sum, sum_i, UPPER, mid, high);
    if (sum[ans] - sum_i > UPPER)
        return ans;
    else
        return high;
	/*
...
//        cout << sum[j] << ' ';
//    }
//    cout << '\n';// You may assume the time complexity here is O(nlog(n)).
	return count;
}
